...
import gaming178.com.mylibrary.lib.util.NetworkUtil;
...
//							while((read=inStream.read(bodyBytes, offset, length))>0)
						String s;
							while(((s=reader.readLine())!=null))
							{
								/*if(length-read==0)
								{*/
									if(null!=respListener&&!s.isEmpty())
									{
										Log.v(TAG,"onSocketResponse---->"+s);
//										respListener.onSocketResponse(new String(bodyBytes));
										respListener.onSocketResponse(s);
									}
