...
public class DateUtils {
     ...
     * @return unix 时间
     */
    public static String getUnixTimeStampMils(String dateTime) {
     ...
     * @return "yyyy-MM-dd"日期的格式化对象
     */
    public static String newLongYMDFormat1() {
        Date d = new Date();
     ...
     * @return
     */
    public static String getDates(String timestampString) {
    ...
    public static String format(Date date, String format) {
        ...
        try {
            ...
            if (null == format || format.equals("")) {// 默认格式化形式
                format = "yyyy-MM-dd";
            }
     ...
     * @return
     */
    public static String getAddDay(int diffDay, String fmt) {
     ...
     * @return
     */
    public static String getAddDay(String datetime, int diffDay, String fmt) {
        SimpleDateFormat sf = new SimpleDateFormat(fmt);
        Date date = DateUtils.format(datetime, DateUtils.Y_M_D);
        Calendar calendar = new GregorianCalendar();
        calendar.setTime(date);
        calendar.add(Calendar.DATE, diffDay);//把日期往后增加一个天数.整数往后推,负数往前移动
    ...
    public static Date formatStringToDate(String argDateStr, String argFormat) throws Exception {
        ...
        if (null == strFormat || strFormat.equals("")) {
            strFormat = Y_M_D;
     ...
     * @return
     */
    public static int getSubTime(String endTime, String startTime) {
        DateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        try {
            Date d1 = df.parse(startTime);
            Date d2 = df.parse(endTime);
            long diff = d1.getTime() / 1000 - d2.getTime() / 1000;// 这样得到的差值是微秒级别
            int sub = (int) diff;
            return sub;
        } catch (Exception e) {
            e.printStackTrace();
    ...
    /**
     * 系统当前时间与传入时间相减 返回相差的秒数
     *
     * @param time 格式 yyyy-MM-dd HH:mm:ss
     * @return
     */
    public static int getSubTime(String time) {
        DateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        try {
     ...
     * @return
     */
    public static String getAddMonth(int diffDay, String fmt) {
        SimpleDateFormat sf = new SimpleDateFormat(fmt);
        Date date = new Date();//取时间
        Calendar calendar = new GregorianCalendar();
        calendar.setTime(date);
        calendar.add(Calendar.MONTH, diffDay);//把日期往后增加一个月.整数往后推,负数往前移动
        date = calendar.getTime();   //这个时间就是日期往后推一天的结果
        return sf.format(date.getTime());
    }
     ...
     * @return
     */
    public static String getAddMonth(String datetime, int diffDay, String fmt) {
        SimpleDateFormat sf = new SimpleDateFormat(fmt);
        Date date = DateUtils.format(datetime, DateUtils.Y_M_D_HMS);
        Calendar calendar = new GregorianCalendar();
        calendar.setTime(date);
        calendar.add(Calendar.MONTH, diffDay);//把日期往后增加一个月.整数往后推,负数往前移动
        date = calendar.getTime();   //这个时间就是日期往后推一天的结果
    ...
    public static void main(String[] args) {
        String newEnd = DateUtils.getAddMonth("2018-04-03 17:48:42", 1, DateUtils.Y_M_D_HMS);
        try {
            System.out.println(newEnd);
            System.out.println(DateUtils.compareEndAndStart(DateUtils.getCurrentDateTime(), "2018-04-03 17:48:42"));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
