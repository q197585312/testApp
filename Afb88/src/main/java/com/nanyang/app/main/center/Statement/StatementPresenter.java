...
//
    ...
    public void getStatementData(String userName) {
        Disposable disposable = mApiWrapper.applySchedulers(Api.getService(ApiService.class).statementData(AppConstant.URL_STAEMENT))
                .subscribe(new Consumer<List<StatementListBean>>() {
                    @Override
                    public void accept(List<StatementListBean> statementListBeen) throws Exception {
                        baseView.onGetStatementListData(statementListBeen);
