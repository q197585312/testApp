...
public class PopJDBTransfer extends BasePopupWindow implements View.OnClickListener {
    String balanceUrl;
    String buyUrl;
    String takeOutUrl;
    VipInfoBean info;
    ...
    public PopJDBTransfer(Context context, View v, int width, int height) {
    ...
    protected void initView(View view) {
        ...
        tv_lottery_balance_name.setText(context.getString(R.string.jdb_game_slots));
        ...
        balanceUrl = WebSiteUrl.JDBBalanceUrl;
        buyUrl = WebSiteUrl.JDBTransferUrl;
        takeOutUrl = WebSiteUrl.JDBTransferUrl;
        ...
        switch (view.getId()) {
            ...
            case R.id.btn_start:
                ...
                String url = WebSiteUrl.JDBLoginUrl + "web_id=" + WebSiteUrl.WebId + "&language=" +
                        Dig88Utils.getLanguage(context) + "&username=" + info.getUsername() + "&token=" + aty.getUserInfoBean().getSession_id() + "&from=app";
                intent.putExtra("url", url);
                intent.putExtra("title", context.getString(R.string.jdb_game_slots));
    ...
    private void request() {
        ...
        if (requestType == typeBuy) {
            ...
            param = "?web_id=" + WebSiteUrl.WebId + "&username=" + info.getUsername()
                    + "&from=1&to=27&amount=" + buyMoney;
        } else if (requestType == typeTakeOut) {
            requestUrl = takeOutUrl;
            param = "?web_id=" + WebSiteUrl.WebId + "&username=" + info.getUsername()
                    + "&from=27&to=1&amount=" + takeOutMoney;
        }
            ...
            public void onSuccess(String result) {
                if (requestType == typeBuy || requestType == typeTakeOut) {
                    ...
                    if (TextUtils.isEmpty(result)) {
                        ...
                        JdbSlotBalanceBean jdbSlotBalanceBean = gson.fromJson(result, JdbSlotBalanceBean.class);
                        tv_poker_balance.setText(jdbSlotBalanceBean.getBalance() + "");
                    }
                }
