 ...
 */
public class ModuleRenameColor {
    ...
    //String文件的地址
//    public static String colorXmlFileNmae = "colors.xml";
    public static String colorXmlFileNmae = "colors.xml";
    public static String renameResPath = workSpaceDir + "res/";
    public static String colorXmlPath = renameResPath + "values/" + colorXmlFileNmae;
    //要添加的前缀 会加上下划线 所以不用额外加上下划线
    public static String modulePrefix = "aaa";

    public static void main(String[] args) {
        List<String> changedList = renameColorXml();
    ...
    //改string.xml里面的key
    public static List<String> renameColorXml() {
        List<String> modifiedList = new ArrayList<>();
        try {
            SAXReader reader = new SAXReader();
            Document doc = reader.read(new File(colorXmlPath));
            ...
            if (modified) {
                //指定文件输出的位置
                FileOutputStream out = new FileOutputStream(colorXmlPath);
    ...
    public static void solveStringReferences(List<String> modifiedList) {
        ...
        int totalSize = modifiedList.size();
        try {
            for (String oldStringValue : modifiedList) {
                ...
                completedCount++;
                System.out.println(completedCount + "/" + totalSize + "  " + oldStringValue + "处理完成");
            }
...
//        System.out.println("doReplaceString oldstr = " + oldstr + " newStr = " + newStr + " line = " + line);
        ...
        if (isJavaFile) {
            ...
            String reg = noNumNoLetterReg + "(R.color." + oldstr + ")" + noNumNoLetterReg;
            ...
            if (matcher.find()) {
                ...
                line = wrappLine.replaceAll(reg, "$1R.color." + newStr + "$3");//只替换前面的字符串
            ...
            String reg = noNumNoLetterReg + "(@color/" + oldstr + ")" + noNumNoLetterReg;
...
//             return  line.matches("R."+endParentPath+"."+oldstr+"[^a-zA-Z0-9]*");
            if (matcher.find()) {
                ...
                System.out.println("替换前：" + line);
                line = wrappLine.replaceAll(reg, "$1@color/" + newStr + "$3");
                line = line.replaceFirst(";", "");
                line = line.substring(0, line.length() - 1);
                System.out.println("替换后：" + line);
...
//            System.out.println("检查" + fileItem + "  " + oldStringValue + "的引用");
    ...
    public static boolean filterFile(String name) {
        //这里只针对color.xml修改，所以要跳过color.xml
        return name != null && (name.endsWith(FILE_JAVA) || name.endsWith(FILE_XML)) && !name.equals(colorXmlFileNmae);
    }
